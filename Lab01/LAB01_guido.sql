...
SELECT * employee_id,last_name sal x 12 ANNUAL SALARY from employess;
...
SELECT (LAST_NAME + ', ' + JOB_ID) As "Empleado y Puesto" FROM EMPLOYEES
...
+  CONVERT(varchar(10), COMMISSION_PCT)+ ', ' +  CONVERT(varchar(10), MANAGER_ID) + ', ' + DEPARTMENT_ID) As "Los Empleados" FROM EMPLOYEES
...
SELECT LAST_NAME, DATEDIFF(MONTH, HIRE_DATE,GETDATE()) as "Meses Trabajados" FROM EMPLOYEES
Order BY 2
...
FROM employees;*/
SELECT LAST_NAME , REPLACE(SALARY,15,'$') AS "Salario" FROM EMPLOYEES

SELECT "Salario" = REPLACE(STR(SALARY, 10), SPACE(1), '$')  FROM EMPLOYEES 

SELECT left(cast(SALARY as varchar(15))+replicate('0',10),10) as "Salario" FROM EMPLOYEES 
...
SELECT SUBSTRING(LAST_NAME,1,8) as "Empleados y sus Salarios" FROM EMPLOYEES;

SELECT SUBSTRING(LAST_NAME,1,8) + REPLACE(' ' , SALARY/1000+1,'*') as "Empleados y sus Salarios" FROM EMPLOYEES;
